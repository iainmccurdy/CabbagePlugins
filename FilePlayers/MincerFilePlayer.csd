...
rslider  bounds(435,200, 70, 90), channel("StackRatio"),range(0.25, 5, 1), text("Ratio"), $SLIDER_DESIGN, visible(0)
...
instr    2    ; non-midi triggered instrument
 ...
 if giReady==1 then
  ...
  if gkStack==1 then ; one layer only
   ...
   ; filters
   a1             buthp        a1, a(cpsoct(gkHPF))
   a1             butlp        a1, a(cpsoct(gkLPF))
   a2             buthp        a2, a(cpsoct(gkHPF))
   a2             butlp        a2, a(cpsoct(gkLPF))
