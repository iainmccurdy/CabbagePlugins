...
opcode playerPulseLayer, a, kiiiikkkkiiiiio
 kSpeed, iwsize, ifn, iLooped, iFileSR, kInterval, kShape, kIntervalMode, kGlissando, iAmpShape, iInterp, iOS, iOS_Type, iNLayers, iCount    xin
 if iOS_Type==1 then ; linear time smear distribution
  iInitPhase        divz                iCount*iOS, iNLayers, 0 ; value within the range 0 to 1
 else                ; random time smear distribution
...
;  iInitPhase        =                   iInitPhase ^ 4
 ...
 if iInterp==1 then                                          ; no interpolation
  aSig             table               aPtr, ifn, 0, 0, 0
 elseif iInterp==2 then                                      ; linear interpolation
  aSig             tablei              aPtr, ifn, 0, 0, 0
 elseif iInterp==3 then                                      ; cubic interpolation
  aSig             table3              aPtr, ifn, 0, 0, 0
 elseif iInterp==4 then                                      ; sinc interpolation
  aSig             tablexkt            aPtr, ifn, kSpeedL, iwsize, 0, 0, 0
 endif
 ...
 kPtr              wrap                kPtr + (ksmps * (iFileSR/sr) * kSpeedL), 0, sr * 1  ; increment pointer
 aMix              =                   0
 if iCount<(iNLayers-1) then
  aMix             playerPulseLayer         kSpeed, iwsize, ifn, iLooped, iFileSR, kInterval, kShape, kIntervalMode, kGlissando, iAmpShape, iInterp, iOS, iOS_Type, iNLayers, iCount + 1
 endif
                   xout                aSig + aMix
endop
...
instr    2    ; Sample triggered by 'play/stop' button
 ...
 aL                playerPulseLayer    kSpeed,iwsize,gitableL,i(kLooped),giFileSR,kInterval,gkShape,kIntervalMode,kGlissando,giAmpShape,i(kInterp),i(kOS),i(kOS_Type),i(kNLayers) 
 if                gichans==2 then
  ...
  aR                playerPulseLayer    kSpeed,iwsize,gitableL,i(kLooped),giFileSR,kInterval,gkShape,kIntervalMode,kGlissando,giAmpShape,i(kInterp),i(kOS),i(kOS_Type),i(kNLayers)
  else
  aR               =                   aL
 endif
