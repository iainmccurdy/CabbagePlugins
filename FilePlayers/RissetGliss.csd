...
; Implements the Risset Glissando effect using a sound file as input.
; It is expected that the file used loops smoothly and works best if this is largely static in pitch.
; It can also be used with a drum loop to create the Risset Beats effect.

; Open File           - browse for a sound file
; Play/Stop           - play/stop the stack of sound files
; Interpolation       - interpolation used when sound file playback is slowed; for very low transpositions higher settings may be sonically beneficial
; Rate                - rate of the glissando
; N. Layers           - number of layers in the glissando stack
; Global Pitch        - global control of all playback speeds/pitches
; Range               - range in octaves across which glissandos slide; can also be controlled remotely using 'Gap Semis'
; Gap Semis           - gap in semitones between adjacent layers
; HPF                 - cutoff frequency in octaves of a high-pass filter applied to audio output
; LPF                 - cutoff frequency in octaves of a low-pass filter applied to audio output
; Level               - output level control
; XY Pad 1            - allows combined control of Range vs Global Pitch
; XY Pad 2            - allows combined control of HPF (high-pass filter cutoff) vs LPF (low-pass filter cutoff)
; Glissando layers can also be triggered from the MIDI keyboard
;  In this case, Global Pitch will be determined by the note played and the output level of that layer will be determined by key velocity

<Cabbage>
form caption("Risset Glissando") size(835, 635), pluginId("RiGl"), colour(10, 20, 20), guiMode("queue")
...
image bounds(0,150,835,160), colour(0,0,0,0), plant("controls")
{
...
rslider    bounds(215,  5, 90, 90), channel("Rate"), text("Rate"), range(-1, 1, 0.01,1,0.0001), colour(50, 60, 60), trackerColour("silver"), textColour("white"), valueTextBox(1)
...
rslider    bounds(440,  5, 90, 90), channel("Range"), text("Range"), range(.1, 8, 1,0.5), colour(50, 60, 60), trackerColour("silver"), textColour("white"), valueTextBox(1)
...
xypad      bounds(  5,265,410,250), channel("X1","Y1"), text("X - Range | Y - Glob.Pitch"), fontColour(0,0,0,0), textColour(200,200,200), ballColour("silver")
xypad      bounds(420,265,410,250), channel("X2","Y2"), text("X - HPF | Y - LPF"), fontColour(0,0,0,0), textColour(200,200,200), ballColour("silver")



keyboard bounds(  5,525, 825, 90)
label    bounds(  5,618, 140, 15), text("Iain McCurdy |2026|"), align("left"), fontColour("silver")
...
opcode RG_LAYER, aa, Skkkiip
Sfile, kRate, kGlobPitch, kRange, iInterp, iNLayers, iCount       xin
iInitPhs           =                   (iCount-1) / iNLayers
aPhs               phasor              kRate/kRange, iInitPhs
aEnv               tablei              aPhs, giWind, 1
kPBSpeed           =                   kGlobPitch * (octave((k(aPhs)*kRange) - (kRange*0.5)))
iNChnls            =                   filenchnls(Sfile)
if iNChnls==1 then
 aSigL             diskin2             Sfile, kPBSpeed,rnd(filelen:i(Sfile)), 1, 0, iInterp
 aSigR             diskin2             Sfile, kPBSpeed,rnd(filelen:i(Sfile)), 1, 0, iInterp
elseif iNChnls==2 then
 aSigL,aSigR       diskin2             Sfile, kPBSpeed, 0, 1, 0, iInterp
endif
aSigL              *=                  aEnv
aSigR              *=                  aEnv
aMixL              =                   0
...
aMixL,aMixR        RG_LAYER            Sfile, kRate, kGlobPitch, kRange, iInterp, iNLayers, iCount+1
endif
                   xout                aSigL+aMixL, aSigR+aMixR
endop
...
aMixL,aMixR        RG_LAYER            Sfile, gkRate, kGlobPitch, gkRange, iInterp, i(gkNLayers)
rireturn

 aMixL             buthp               aMixL, gaHPF
 aMixR             buthp               aMixR, gaHPF
 aMixL             butlp               aMixL, gaLPF
 aMixR             butlp               aMixR, gaLPF

 aEnv              transegr            0, 1, -4, 1, 1, -4, 0
 aMixL             *=                  aEnv
 aMixR             *=                  aEnv
 
                   outs                aMixL * gaLevel * iVel, aMixR * gaLevel * iVel
