...
; DynamicLooper.csd (previously Table3FilePlayer.csd)
; Written by Iain McCurdy, 2014

; tablexkt is used for higher quality at low transpositions and for anti-aliasing
...
; L O O P 
; Open File     -  browse for a sound file. You can also drag and drop a file onto the GUI.
; Play/Stop     -  start/stop playback
; Looping Mode  -  Forward, Backward, Fwd./Bwd.
; Window        -  if activated, an amplitude window is overlaid onto each loop repetition. 
;                  This can prevent clicks dur to discontinuities at loop points
; Mouse Gate    -  if activated, mouse left button will turn on and off sound with an amplitude envelope. ('Play/Stop' needs to be active.)
;                  The attack and release times of this envelope are set using 'Att. Time' and 'Rel. Time'
; Y Controls    -  the mouse's Y vertical position over the waveform viewer can be used to control other parameters.
; (Y Control) Transposition - if activated, vertical mouse movement over the waveform viewer will also change transposition/speed
; (Y Control) Filters - if activated, vertical mouse movement over the waveform viewer will also change 
;                       the cutoff frequencies of low-pass and high pass filters. Resonance of the filters can be changed.

; Start         -  (slider above the waveform viewer) start point of the loop.      
; End           -  (slider below the waveform viewer) end point of the loop.
;                   The whole loop can also be dragged forwards and backwards 
;                    by clicking and dragging on the selected area within the waveform viewer.
; Portamento    -  lag time that is applied to changes in loop start/end points and position

; S P E E D        (either method of defining speed/pitch can be used)
; Semitones     -   speed expressed as a transposition in semitones; 0 = normal speed
; Speed         -   speed as a ratio to normal speed; 1 = normal speed

; E N V E L O P E Amplitude envelope applied to each start and stopping of a note, either by the GUI button or via MIDI
; Att. Time     -   envelope attack time in seconds
; Rel. Time     -   envelope release time in seconds

; C O N T R O L
; MIDI Ref.     -   MIDI note that will produce unison/normal speed playback
; Detect        -   detect pitch of the same (should be a steady note) and apply this to the MIDI Ref.
; Pitch Bend    -   range of effect of a MIDI keyboard's pitch bend wheel
; Level         -   output level
...
checkbox   bounds( 10,100, 95, 14), channel("MouseGate"), text("Mouse Gate"), colour("yellow"), fontColour:0("white"), fontColour:1("white")
...
groupbox   bounds( 125, 25,120, 80), plant("Y to Trans."), text("Y Controls"), fontColour("white")
 {
 checkbox   bounds( 10, 28,110, 15), channel("Y2Trans"), text("Transposition"), value(0), fontColour:0("white"), fontColour:1("white")
...
kYNorm   limit   1 - ((kMouseY-iBounds[1]) / iBounds[3]), 0, 1
if kMouseDownLeft==1 && cabbageGetValue:k("Y2Trans")==1 then
          cabbageSetValue "transpose", scale:k(kYNorm,72,-72), changed:k(kYNorm)
endif
...
instr    2    ; Sample triggered by 'play/stop' button
 ...
 if giReady==1 then                                              ; i.e. if a file has been loaded
  ...
  if gichans==1 then                                              ; if mono...
   a1              tablexkt            aphasor, gitableL, kspeed, 16, 1
   a2              =                   a1
  elseif gichans==2 then                                          ; otherwise, if stereo...
    a1              tablexkt            aphasor, gitableL, kspeed, 16, 1
    a2              tablexkt            aphasor, gitableR, kspeed, 16, 1
  endif
  ...
  if cabbageGetValue:k("MouseGate")==1 then
   a1              *=                  aMEnv
   a2              *=                  aMEnv
  endif
