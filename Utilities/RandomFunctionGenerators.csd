...
<Cabbage>
form caption("Random Function Generators"), size(960, 335), pluginId("RaFu"), guiRefresh(32), colour(45,45,50), guiMode("queue")
...
instr    1
 ...
 if metro(kROU)==1 then                                     ; limit updates (both to the table and to the GUI) using a metronome
  ...
  kNdx    wrap       kNdx+1,0,iLen                          ; increment index and wrap ot zero if it exceeds table length
          ...
          cabbageSet k(1),"wiper","bounds", ((kNdx/iLen) * giTableWidth), 0, 1, 150
 endif
