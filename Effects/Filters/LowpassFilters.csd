...
; K35_lpf
; bob
; mvmfilter  ; doesn't seem to be working in Cabbage yet?...
...
combobox bounds( 15, 43,100, 20), channel("input"), align("centre"), value(2), text("Live","Sawtooth","Noise","Chord"), align("centre")
label    bounds( 15, 70,100, 12), text("Type"), fontColour("white"), align("centre")
combobox bounds( 15, 83,100, 20), align("centre"), value(18), text("12dB/oct","24dB/oct","36dB/oct","48dB/oct","60dB/oct","diode_ladder","moogladder","moogladder2","moogvcf","moogvcf2","rezzy","lpf18","lowres","tbvcf","mvclpf1","mvclpf2","mvclpf3","mvclpf4","bqrez","spf","skf","svn","vclpf","zdf_1pole","zdf_2pole","zdf_ladder","K35_lpf","bob","mvmfilt"), channel("type")
...
rslider  bounds(255, 25, 70, 90), channel("sat"), text("Sat."), range(0.01,1.00,0.6), visible(0), $SLIDER_DESIGN
rslider  bounds(190, 25, 70, 90), channel("tau"), text("Tau"), range(0.01,8.00,0.1), visible(0), $SLIDER_DESIGN
...
instr    2 ; filter (always on)
    ...
    if changed:k(ktype)==1 then
     if ktype==6 || ktype==7 || ktype==8 || ktype==9 || ktype==10 || ktype==11 || ktype==12 || ktype==13 || ktype==14 || ktype==15 || ktype==16 || \
        ktype==17 || ktype==18 || ktype==19 || ktype==20 || ktype==21 || ktype==22 || ktype==23 || ktype==25 || ktype==26 || ktype==27 || ktype==28 then
    ...
    ; show hide saturation control (bob)
    if changed:k(ktype)==1 then
     if ktype==28 then
                  cabbageSet      k(1),"sat","visible",1
     else
                  cabbageSet      k(1),"sat","visible",0
     endif
    endif

    ; show hide tau control (mvmfilt)
    if changed:k(ktype)==1 then
     if ktype==29 then
    ...
    /* FILTER */
    if ktype==6 then
     aFiltL       diode_ladder    aL*5,acf,kres*17 ; (loses power so the input is boosted)
     aFiltR       diode_ladder    aR*5,acf,kres*17
    elseif ktype==7 then
    ...
    elseif ktype==27 then
      ...
      aFiltL    *=         1 / (1 + kdist)
      aFiltR    *=         1 / (1 + kdist)
    elseif ktype==28 then
      aFiltL    bob        aL, acf, kres*4, ksat
      aFiltR    bob        aR, acf, kres*4, ksat
      ; gain compensate
      aFiltL    /=         ksat
      aFiltR    /=         ksat
    elseif ktype==29 then ; doesn't seem to be working in Cabbage yet?...
      aFiltL    mvmfilter  aL, acf, ktau
      aFiltR    mvmfilter  aR, acf, ktau
    else
     if changed:k(ktype)==1 then
