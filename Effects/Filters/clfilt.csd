...
; Sound file playback
; Link         -   link sound file playback speed to filter cutoff frequency. Unity scaling is set when Link button is pressed.
; Speed (horizontal slider) - sound file playback speed manual control
...
checkbox   bounds( 10, 57, 95, 18), channel("PlayStop"), text("Play/Stop"), colour("lime"), fontColour:0("white"), fontColour:1("white"), colour:0( 90, 90,0), colour:1(255,255,0), corners(3)
checkbox   bounds( 10, 80, 95, 18), channel("link"), text("Link"), colour("lime"), fontColour:0("white"), fontColour:1("white"), colour:0( 90, 90,0), colour:1(255,255,0), corners(3)
hslider    bounds( 10,105, 90, 10), channel("speed"),    range(0.25,2,1,0.5);, $SLIDER_STYLE ;, text("Speed")
...
instr 101 ; PLAY LOADED SOUND FILE
 if trigger:k(gklink,0.5,0)==1 then
  reinit SPEED
 endif
 SPEED:
 if gklink==1 then
  kspeed = gkspeed * gkcf/i(gkcf)
 else
  kspeed = gkspeed 
 endif
 rireturn
 if  giChans==1 then ; mono
  gaInL            diskin2             gSfilepath, kspeed, 0, 1
  gaInR            =                   gaInL
 else                ; stereo
  gaInL,gaInR      diskin2             gSfilepath, kspeed, 0, 1
 endif
