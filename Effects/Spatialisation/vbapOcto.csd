...
<Cabbage>
form caption("Vector-Based Amplitude Panning for Octophonic Surround") size(820,492), pluginId("VbOc"), guiMode("queue"), colour(40,40,40)

; XY panel
image bounds(  0, 0,400,400), colour(40,40,40), channel("panel"), outlineThickness(1) ; xy panel

; title
label bounds(  0,120,400,160), text("VBAP"), fontColour("white"), alpha(.3), colour(0,0,0,0), channel("title")
...
; location  widget
image bounds(193,192,16,16), colour("White"), shape("ellipse"), channel("widget"), alpha(0.8), visible(0)     ; panning widget

; xy pad
xypad bounds( -10, -10,420,484), colour(0,0,0,0), ballColour("White"), channel("X","Y"), rangeX(-2,2,0), rangeY(-2,2,0), alpha(0.7), fontColour(0,0,0,0)
...
nslider bounds(415,  5, 70, 30), channel("azim1"), text("Azim.1"), range(-360,360,-22.5)
nslider bounds(490,  5, 70, 30), channel("azim2"), text("Azim.2"), range(-360,360,22.5)
nslider bounds(565,  5, 70, 30), channel("azim3"), text("Azim.3"), range(-360,360,-67.5)
nslider bounds(640,  5, 70, 30), channel("azim4"), text("Azim.4"), range(-360,360,67.5)
nslider bounds(415, 45, 70, 30), channel("azim5"), text("Azim.5"), range(-360,360,-112.5)
nslider bounds(490, 45, 70, 30), channel("azim6"), text("Azim.6"), range(-360,360,112.5)
nslider bounds(565, 45, 70, 30), channel("azim7"), text("Azim.7"), range(-360,360,-157.5)
nslider bounds(640, 45, 70, 30), channel("azim8"), text("Azim.8"), range(-360,360,157.5)

; rotate
label   bounds(730,  5, 51, 13), text("ROTATE"), align("centre")
button  bounds(730, 20, 25, 22), channel("DecrAz"), text("<"), latched(0), corners(5)
button  bounds(757, 20, 25, 22), channel("IncrAz"), text(">"), latched(0), corners(5)

label    bounds(720, 45, 70, 13), text("PRESET")
combobox bounds(720, 57, 70, 22), channel("Preset"), items("St. Pairs","Ring 1","Ring 2"), value(1)

hslider  bounds(415, 90,390, 15), channel("width"), text("Width"), range(0,1,0.5), valueTextBox(1)
hslider  bounds(415,120,390, 15), channel("spread"), text("Spread"), range(0,100,0,1,1), valueTextBox(1)
hslider  bounds(415,150,390, 15), channel("scale"), text("Scale"), range(0,92,24,1,1), valueTextBox(1)

label    bounds(420,185, 70, 13), text("CONTROL"), align("centre")
combobox bounds(420,200, 70, 20), items("Mouse","LFO"), channel("control"), value(1)
rslider  bounds(520,180, 50, 50), channel("Gain"), text("Gain"), range(0,10,1,0.5,0.001), valueTextBox(0)

; meters
image   bounds(580,190,135, 42), colour(0,0,0,0)
{
...
checkbox bounds(725,185, 70, 15), text("Doppler"), channel("DopOnOff"), range(1,50,1,0.5)
nslider  bounds(725,205, 60, 30), text("Dop.Scale"), channel("DopScale"), range(0.1,5,1,0.5)

; lfo controls
image    bounds(420,250,390,145), colour(0,0,0,0), channel("lfo"), outlineThickness(1), alpha(0.2)
{
...
; file player
image bounds( 10,410,800, 70) colour(0,0,0,0)
{
...
label      bounds( 10,473,690, 14), text("Iain McCurdy |2024|"), align("left")
...
;                       1      2     3      4      5       6      7      8
giPairs ftgen 1,0,8,-2, -22.5, 22.5, -67.5, 67.5,  -112.5, 112.5,-157.5, 157.5
giRing1 ftgen 2,0,8,-2, 0,     45,    90,   135,   180,   -135,   -90,    -45
giRing2 ftgen 3,0,8,-2, -22.5, 22.5,  67.5, 112.5, 157.5, -157.5, -112.5, -67.5
...
cabbageSet changed:k(kcontrol), "widget", "visible", kcontrol-1
cabbageSet changed:k(kcontrol), "X", "visible", 1 - (kcontrol-1)

cabbageSet changed:k(kcontrol), "Spk1", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk2", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk3", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk4", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk5", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk6", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk7", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "Spk8", "alpha", (1 - (kcontrol-1)) * 0.7 + 0.3
cabbageSet changed:k(kcontrol), "title", "alpha", (1 - (kcontrol-1)) * 0.3 + 0.1
...
if kcontrol==1 then ; mouse control
 ...
 iXYPadBounds[]   cabbageGet      "X", "bounds"
 iXYPadX          =               iXYPadBounds[0]
 iXYPadY          =               iXYPadBounds[1]
 iXYPadWid        =               iXYPadBounds[2]
 iXYPadHei        =               iXYPadBounds[3] 
                 cabbageSet      changed:k(kX,kY), "widget", "bounds", (kX/4 + 0.5)*iXYPadWid*0.9, (1 - (kY/4 + 0.5))*iXYPadHei*0.78, iWidgRad*2, iWidgRad*2 ; move the widget to the new location whenever the mouse coordinates have changed  
  
  ; smooth XY pad movements
...
kazim1             init                -22.5 ; set initial values for i-time pass
kazim2             init                22.5
kazim3             init                -67.5
kazim4             init                67.5
kazim5             init                -112.5
...
iSpkrs[]           fillarray           i(kazim1), i(kazim2), i(kazim3), i(kazim4), i(kazim5), i(kazim6), i(kazim7), i(kazim8)
                   vbaplsinit          idim, lenarray:i(iSpkrs), iSpkrs
...
aOuts[]            vbap                asig * ampdbfs(kdB) * kGain, kaz , kel, kspread
rireturn

 ; load preset
 kPreset cabbageGetValue "Preset"
 if changed:k(kPreset)==1 then
  cabbageSetValue   "azim1", tableikt:k(0,kPreset)
  cabbageSetValue   "azim2", tableikt:k(1,kPreset)
  cabbageSetValue   "azim3", tableikt:k(2,kPreset)
  cabbageSetValue   "azim4", tableikt:k(3,kPreset)
  cabbageSetValue   "azim5", tableikt:k(4,kPreset)
  cabbageSetValue   "azim6", tableikt:k(5,kPreset)
  cabbageSetValue   "azim7", tableikt:k(6,kPreset)
  cabbageSetValue   "azim8", tableikt:k(7,kPreset)
 endif
...
if kDopOnOff==1 then
 /*
 kmicposition      =                   0
 aOuts[0]          doppler             aOuts[0], kDist1*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[1]          doppler             aOuts[1], kDist2*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[2]          doppler             aOuts[2], kDist3*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[3]          doppler             aOuts[3], kDist4*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[4]          doppler             aOuts[4], kDist5*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[5]          doppler             aOuts[5], kDist6*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[6]          doppler             aOuts[6], kDist7*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 aOuts[7]          doppler             aOuts[7], kDist8*kDopScale, kmicposition ;[, isoundspeed, ifiltercutoff] 
 */
 ;creating the doppler using vdelay may be more efficient
 ic                =                   343 ; speed of sound in air
 aOuts[0]          vdelay              aOuts[0], a(kDist1*kDopScale*1000/ic), 1000
 aOuts[1]          vdelay              aOuts[1], a(kDist2*kDopScale*1000/ic), 1000
 aOuts[2]          vdelay              aOuts[2], a(kDist3*kDopScale*1000/ic), 1000
 aOuts[3]          vdelay              aOuts[3], a(kDist4*kDopScale*1000/ic), 1000
 aOuts[4]          vdelay              aOuts[4], a(kDist5*kDopScale*1000/ic), 1000
 aOuts[5]          vdelay              aOuts[5], a(kDist6*kDopScale*1000/ic), 1000
 aOuts[6]          vdelay              aOuts[6], a(kDist7*kDopScale*1000/ic), 1000
 aOuts[7]          vdelay              aOuts[7], a(kDist8*kDopScale*1000/ic), 1000
 
endif



                   outo                aOuts[0],aOuts[1],aOuts[2],aOuts[3],aOuts[4],aOuts[5],aOuts[6],aOuts[7] ; octophonic output
;outs a1,a2
