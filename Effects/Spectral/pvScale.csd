...
<Cabbage>
form caption("pvscale Pitch Shifter") size(690,370), pluginId("scal") guiMode("queue")
image                         bounds(0, 0, 690,370), colour("DarkSlateGrey"), outlineColour("silver"), outlineThickness(4)
...
label    bounds( 10, 15, 75, 13), text("Audio Input"), $LABEL_DESIGN
...
combobox bounds( 10, 70, 75, 20), channel("IntervalMode"), text("Semitone","Ratio","MIDI"), value(1), $COMBOBOX_DESIGN
...
keyboard bounds(  5,265,680, 85)

label    bounds( 10,353,120, 11), text("Iain McCurdy |2012|"), align("left"), fontColour("silver")
</Cabbage>
...
0dbfs        =    1    ; MAXIMUM AMPLITUDE
             ...
             massign 0,11 ; all MIDI channels to instr 1
...
if trigger:k(kIntervalMode,2.5,1)==1 then
 event "i",11,0,-1
endif

if trigger:k(kIntervalMode,2.5,0)==1 then
 turnoff2 11,0,0
endif
endin
...
instr    11
    ...
    if ivalue==1 then ; MIDI
     if iIntervalMode!=3 then
      turnoff
     endif
     kscale        =        cpsmidi:i()/cpsmidinn:i(60)
